...
class MenuExample:
	def __init__(self):
		...
		# Init the menu-widget, and remember -- never show() the menu
		# widget!!
		# This is the menu that holds the menu items, the one that will
		# pop up when you click the "Root Menu" in the app
		menu = gtk.Menu()
		...
		# catch the "clicked" signal on each of the menu items and setup
		# a callback for it, but it's omitted here to save space.
		for i in range(3):
			...
			# Do something interesteing when the menuitem is selected
			menu_item.connect("activate", self.menuitem_response, buf)
			# Show the widget
			menu_item.show()

		# This is the root menu, and will be the label
		# displayed on the menu bar. THere won't be a signal handler
		# attached, as it only pops up the rest of the menu when pressed
		...
		# Create a menu-bar to hold the menus and add it to our main
		# window
		...
		# And finally we append the menu-item to the menu-bar -- this
		# is the "root" menu-item I have been raving about =)
	...
	# Respond to a button-press by posting a menu pressed in as widget.
	#
	# Note that the "widget" argument is the menu being posted, NOT the
